...
class TimeShap(evaluate_model):
    ...
    def group(self,label,path):
        ...
        event_dict = {'path': path+'event_all_tf.csv', 'rs': 100, 'nsamples': len(data)}
        feature_dict = {'path': path+'feature_all_tf.csv', 'rs': 100, 'nsamples': len(data), 'feature_names': self.feature_names, 'plot_features': self.plot_feats, }
...
class Shap_Analysis(evaluate_model):
    ...
    def plot_Shap(self):
        ...
        self.df = self.df.drop(columns=['id','label','timestamp'])
        shap_values = explainer.shap_values(self.df)
        plt.figure()
        shap.summary_plot(shap_values, self.df.values, plot_type="bar", class_names= ['immune'], feature_names = self.df.columns)
        plt.figure()
        shap.summary_plot(shap_values, self.df.values, feature_names = self.df.columns)
...
LSTM_DyanRout = LSTM_DyanRout_model
LSTM_DyanRout.load_weights(path2LSTM_with_Dyna_Routing)    
random_number = 42
tolerance = 0
top_x_feats = 15
top_x_events = 5
row = 10
group_label= 1
Explainer_TimeShap = TimeShap(LSTM_DyanRout,X_12M,y_12M_class,feature_names_longformat,random_number,tolerance,top_x_feats,top_x_events, plot_feats)
local_report = Explainer_TimeShap.Individual(row)


path = 'D:/UHN/Covid19 Vaccination/Github_submission/LSTM_DynamicRouting_TimeSHAP_20240322/'
prun_stats, global_plot = Explainer_TimeShap.group(group_label,path)
Explainer_TimeShap.plot_TimeShap(path)
...
with open(filename, 'rb') as reader:
     regr = cPickle.load(reader)
Explainer_Shap = Shap_Analysis(regr,X_12M,y_12M_class,feature_names_longformat)     
Explainer_Shap.plot_Shap()


    
path2LSTM = 'C:\\Users\\azarf\\Downloads\\LSTM\\'
LSTM = tf.keras.models.load_model(path2LSTM)
Explainer_TimeShap_LSTM = TimeShap(LSTM,X_12M,y_12M_class,feature_names_longformat,random_number,tolerance,top_x_feats,top_x_events, plot_feats)
#local_report = Explainer_TimeShap_LSTM.Individual(row)
path = 'LSTM_TimeSHAP_20240322/'
prun_stats, global_plot = Explainer_TimeShap_LSTM.group(group_label,path)

Explainer_TimeShap_LSTM.plot_TimeShap(path)
alt.data_transformers.disable_max_rows()
global_plot.show()
